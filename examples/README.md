...
Each CSV and JSON pair describes the same games, and both formats accept the same values; JSON is stricter about types (see below). A CSV import first shows how its columns were matched so you can correct them; a JSON import is read directly.
...
- **`store_game_id`**: the game's id on its store; for Steam, the number in `store.steampowered.com/app/<id>`.
  ...
  - **Stable identity:** your edits and hidden flags are attached to store + id. Without an id, the row's position in the file stands in for it, so re-importing a file with rows added or reordered can move your edits onto other games. Those position-based ids also differ between a CSV and a JSON import, so switching formats detaches edits too.
...
| `status` | `playStatus` | `played`, `unplayed`, `unknown`. Also read: `playing`, `completed`, `finished` or `beaten` as played; `not played`, `never played`, `not started` or `backlog` as unplayed; `incomplete` or `unfinished` as unknown, since not finishing says nothing about starting | `unknown` |
| `playtime_minutes` | `playtimeMinutes` | A number of minutes. In a CSV, a playtime column whose header does not contain "minute" (`hours`, `hours_played`, `playtime`) is read as **hours**, and `14.5` becomes 870 minutes. Steam's own `playtime_forever` column is minutes | No playtime |
...
- **CSV headers are matched loosely**, ignoring case, spaces and punctuation, and common alternatives are recognised: `name` or `game` for the title, `hours_played` for playtime, `app_id` or `id` for the store id, `genre`, `year`, `developers`, `publishers` and `note`. Title, store, store id, platform, status and playtime can also be reassigned on the confirmation screen before the import is written.
