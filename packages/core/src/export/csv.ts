...
export function toCsv(games: ExportedGame[]): string {
  const rows = games.map((game) => ({
    ...
    critic_score_source: game.criticScoreSource ?? '',
    steam_review_percent: game.steamReviewPercent ?? '',
    steam_review_count: game.steamReviewCount ?? '',
    steam_review_label: game.steamReviewLabel ?? '',
    store_url: game.storeUrl ?? '',
    steam_app_id: game.steamAppId ?? '',
    steam_app_pinned: flag(game.steamAppPinned),
    pcgamingwiki_checked_at: game.pcgamingwikiCheckedAt ?? '',
